...
static bool ResolveImportByOrdinalUnwrapper(const char *image, uint32_t ordinal,
                                            uint32_t *result) {
  *result = resolve_import_by_ordinal_proc(image, ordinal);
  return *result != 0;
}

static bool ResolveImportByNameUnwrapper(const char *image, const char *export,
                                         uint32_t *result) {
  *result = resolve_import_by_name_proc(image, export);
  return *result != 0;
}

bool LoadDLL(const uint8_t *raw_data, uint32_t raw_data_size,
...
bool RelocateDLL(uint32_t new_base) { return DLLRelocate(&context, new_base); }

uint8_t *LoadedImage() { return context.output.image; }
...
void FreeDLL() { DLLFreeContext(&context, false); }
